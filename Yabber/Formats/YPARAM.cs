...
namespace Yabber
{
    static class YPARAM
    {
        ...
        private static Dictionary<YBUtil.GameType, Dictionary<string, PARAMDEF>> ParamdefStorage { get; set; } =
            new Dictionary<YBUtil.GameType, Dictionary<string, PARAMDEF>>();
        ...
        public static bool Unpack(this PARAM param, string sourceFile, YBUtil.GameType game)
        {
            ...
            {
                Console.WriteLine($"Param type {paramType} not found in {YBUtil.GameNames[game]} paramdefs.");
            ...
            xw.WriteElementString("game", YBUtil.GameNames[game]);
            ...
            foreach (YPARAMRow row in rows)
            {
                ...
                var csvValues = new List<string>();
                {
                    ...
                    if (cellStyle == CellStyle.CSV)
                        xw.WriteString(YBUtil.DelimitedString.Join(csvValues));
                }
                xw.WriteEndElement();
            }
        ...
        public static bool Repack(string sourceFile)
        {
            ...
            // Enum.TryParse(xml.SelectSingleNode("param/game")?.InnerText ?? "", out YBUtil.GameType game);
            ...
            foreach (XmlNode xmlRow in xml.SelectNodes("param/rows/row"))
            {
                ...
                var csv = cellStyle == CellStyle.CSV ? YBUtil.DelimitedString.Split(xmlRow.InnerText) : null;
        ...
        public static void PopulateParamdef(YBUtil.GameType game)
        {
            if (ParamdefStorage[game].Count > 0)
                return;

            var gameName = YBUtil.GameNames[game];
            var paramdefPath = $@"{YBUtil.GetExeLocation()}\Paramdex\{gameName}\Defs";
        ...
        public static void PopulateNames(YBUtil.GameType game, string paramName)
        {
            if (NameStorage[game].ContainsKey(paramName) && NameStorage[game][paramName].Count > 0)
                return;

            var gameName = YBUtil.GameNames[game];
            var namePath = $@"{YBUtil.GetExeLocation()}\Paramdex\{gameName}\Names\{paramName}.txt";
        ...
        static YPARAM()
        {
            foreach (YBUtil.GameType game in (YBUtil.GameType[])Enum.GetValues(typeof(YBUtil.GameType)))
            {
